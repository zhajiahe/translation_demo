...
def main(args):
    ...
    def translate():
        ...
        prefix_src, _ = preprocess(prefix_src) if prefix_src != "" else (None, None)
        print(prefix_src)
        inputs, line_sep = preprocess(inputs, 'en')
        ...
        for batch in make_batches(inputs, args, task, max_positions, encode_fn):
            ...
            prefix_tokens = tgt_dict.encode_line(prefix_src[0]).cuda().unsqueeze(0).long()[:,:-1] if prefix_src  else None
            translations = task.inference_step(generator, models, sample, constraints=constraints, prefix_tokens = prefix_tokens)
        ...
        return output
    app.run(host='192.168.126.129',port='2020')
